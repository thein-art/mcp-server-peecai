...
export interface Brand {
  ...
  domains?: string[];
  is_own: boolean;
  /** Alternative names for the brand. */
  aliases?: string[];
  /** Hex color of the brand (e.g. "#3b82f6"). */
  color?: string;
  /** How the brand was added. Absent for brands created before this field was tracked. */
  source?: "manual" | "suggestion" | "onboarding";
}

/** A brand suggestion that can be accepted to create a brand, or rejected. */
export interface BrandSuggestion {
  id: string;
  name: string;
  domains: string[];
  /** Number of chats that referenced this brand candidate. */
  chat_count: number;
}
...
export interface ModelChannel {
  ...
  is_active: boolean;
  /** ISO 3166-1 alpha-2 country codes that cannot be used with this channel. Chats requested for these countries are not created. */
...
export interface DomainReportRow {
  ...
  /** Total number of distinct URL retrievals from this domain across all chats. */
  retrieval_count?: number;
  /** Total number of citations from this domain. */
  citation_count?: number;
  /** Number of distinct chats in which at least one URL from this domain was retrieved. */
  retrieved_chat_count?: number;
  /** Total number of chats in scope (denominator for retrieval_rate / retrieval_percentage). */
...
export interface UrlReportRow {
  ...
  citation_count: number;
  /** Number of distinct chats that retrieved this URL. */
  retrieval_count?: number;
  /** @deprecated Use `retrieval_count` instead. */
  retrievals?: number;
...
/** Distribution of audience archetypes; integer counts that should sum to 100. */
export interface AudienceDistribution {
  simpleRecommendationSeeker: number;
...
export interface TargetMarket {
  ...
  location: string;
  /** OpenStreetMap relation/way/node ID. */
  osmId?: string;
}

/** Project brand profile: industry, identity, markets, and audience distribution. */
export interface ProjectProfile {
  ...
  /** Read-only display name (cannot be set via PUT). */
  name?: string;
  /** Read-only flag indicating profile was seeded from a prepared template. */
