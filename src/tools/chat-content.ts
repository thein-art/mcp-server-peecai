...
import { chatContentOutput } from "../schemas.js";
...
export function registerChatContentTool(server: McpServer, client: PeecApiClient) {
  server.registerTool(
    "get_chat_content",
    {
      ...
      outputSchema: chatContentOutput,
      annotations: { readOnlyHint: true, idempotentHint: true, destructiveHint: false, openWorldHint: false },
    },
    async ({ chat_id, project_id }, extra) => {
      try {
        const data = await client.getRaw<ChatContent>(`/chats/${encodeURIComponent(chat_id)}/content`, {
